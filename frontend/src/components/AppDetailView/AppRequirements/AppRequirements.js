...
class AppRequirements extends Component {
    ...
    componentWillMount() {
        this.setState({
            AppRequirement: this.props.requirement
        })
    }
    ...
    handleOk = () => {
        ...
        axios.patch(`http://127.0.0.1:8000/api/projects/${this.props.id}/`, {
            requirements: this.state.AppRequirement,
        })
            .then(response => {
                console.log(response);
                if (response.statusText === "OK") {
                    this.setState({
                        AppRequirement: response.data.requirements,
                        confirmLoading: false,
                        visible: false,
                    })
                }
    ...
    editField = (name, id, index) => {
        ...
        const updateState = {
            ...this.state.controls,
            name: {
                value: this.props.requirement[index].name
    ...
    inputChangedHandler = (event, controlName) => {
        ...
        const newData = this.state.AppRequirement.map(obj => {
        ...
        this.setState({ controls: updateControls, AppRequirement: newData });
    };
        ...
        return (
            <div className="row col-md-12 pt-5 pl-5">
                <div className="Show-Edit-Btn row col-md-12 p-4">
                    <SecondaryHeading>Requirements</SecondaryHeading>
                    {this.props.isAdmin ?
                        <Button shape="circle"
                    ...
                    <AsyncModal visible={this.state.visible}
                        ...
                        <div className='row'>
                            ...
                            {this.state.AppRequirement.map((el, index) => (
                ...
                <div className="row col-md-12 ml-1">
                    <UnOrderList list={this.state.AppRequirement} fa="fas fa-check-circle" />
                </div>
            </div>
        )
    }
